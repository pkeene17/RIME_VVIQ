...
VVIQdf %>% group_by(subjID) %>% summarise_at(vars(response),list(mean=mean)) %>%
  ggplot(aes(x=subjID,y=mean)) + geom_point() + 
  ggtitle("Mean Vividness Questionaire Ratings by Subject")
```
