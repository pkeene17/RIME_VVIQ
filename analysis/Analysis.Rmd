...
stim_dprimedf %>% group_by(retrieval) %>% summarise_at(vars(dprime),list(dmean=mean)) %>% ggplot(aes(x=retrieval,y=dmean)) +geom_bar(stat="identity")

#plot hr and fa by retrieval status
stim_dprimedf %>% group_by(retrieval) %>% summarise_at(vars(hr),list(hr_mean=mean)) %>% ggplot(aes(x=retrieval,y=hr_mean)) +geom_bar(stat="identity")

#plot hr and fa by retrieval status
stim_dprimedf %>% group_by(retrieval) %>% summarise_at(vars(fa),list(fa_mean=mean)) %>% ggplot(aes(x=retrieval,y=fa_mean)) +geom_bar(stat="identity")
```
...
dprimedf %>% ggscatter(x='fa',y='VVIQ',add='reg.line',cor.coef=TRUE,cor.method='pearson')

#this is a little more reasonable
#cor.test(dprimedf$dprime,dprimedf$VVIQ,method='pearson')
dprimedf %>% ggscatter(x='dprime',y='VVIQ',add='reg.line',cor.coef=TRUE,cor.method='pearson')
...
dprimedf %>% group_by(VVIQ_group) %>% summarise_at(vars(fa),list(fa_mean=mean)) %>% 
  ggplot(aes(x=VVIQ_group,y=fa_mean)) + geom_bar(stat='identity')
#dprime
dprimedf %>% group_by(VVIQ_group) %>% summarise_at(vars(dprime),list(dprime_mean=mean)) %>% 
  ggplot(aes(x=VVIQ_group,y=dprime_mean)) + geom_bar(stat='identity')
```
