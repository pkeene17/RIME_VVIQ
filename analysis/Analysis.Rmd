...
stim_dprimedf <- data.frame(matrix(ncol=5))
colnames(stim_dprimedf) <- c('subjID','hrRet','faRet','hrNoRet','faNoRet')
for(isubj in seq(from=1,to=nsubj))
{
  ...
  hrRet <- sum(posttestdf$sureHits[indexRet])/sum(posttestdf$condLabels[indexRet]=="Old")
  faRet <- sum(posttestdf$sureFA[indexRet])/sum(posttestdf$condLabels[indexRet]!="Old")
  hrNoRet <- sum(posttestdf$sureHits[indexNoRet])/sum(posttestdf$condLabels[indexNoRet]=="Old")
  faNoRet <- sum(posttestdf$sureFA[indexNoRet])/sum(posttestdf$condLabels[indexNoRet]!="Old")
  stim_dprimedf <- rbind(stim_dprimedf,c(isubj,hrRet,faRet,hrNoRet,faNoRet))
}
stim_dprimedf <- stim_dprimedf[1:nsubj+1,] #artifact of not being tidy
#stim_dprimedf <- stim_dprimedf[stim_dprimedf$subjID %!in% exclude_subj,]
stim_dprimedf <- mutate(stim_dprimedf,dprimeRet=norminv(stim_dprimedf$hrRet,0,1)-norminv(stim_dprimedf$faRet,0,1))
stim_dprimedf <- mutate(stim_dprimedf,dprimeNoRet=norminv(stim_dprimedf$hrNoRet,0,1)-norminv(stim_dprimedf$faNoRet,0,1))

stim_dprimedf %>% summarise_at(vars(dprimeRet,dprimeNoRet),list(mean=mean)) %>% ggplot(aes(x=c('ret','no ret'),y=c(dprimeRet_mean,dprimeNoRet_mean))) +geom_bar(stat="identity")
```
