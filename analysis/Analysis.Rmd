...
for(isubj in seq(from=1,to=nsubj))
{
  ...
  faRet <- as.numeric(sum(posttestdf$sureFA[indexRet])/sum(posttestdf$condLabels[indexRet]!="Old"))
...
for(isubj in seq(from=1,to=nsubj))
{
  ...
  faNoRet <- as.numeric(sum(posttestdf$sureFA[indexNoRet])/sum(posttestdf$condLabels[indexNoRet]!="Old"))
...
stim_dprimedf %>% group_by(retrieval) %>% summarise_at(vars(dprime),list(dmean=mean)) %>%
  ggplot(aes(x=retrieval,y=dmean,fill=retrieval)) +geom_bar(stat="identity")+ggtitle('dprime retreived vs not retreived stimuli')
...
stim_dprimedf %>% group_by(retrieval) %>% summarise_at(vars(hr),list(hr_mean=mean)) %>% 
  ggplot(aes(x=retrieval,y=hr_mean,fill=retrieval)) +geom_bar(stat="identity")+ggtitle('hr retreived vs not retreived stimuli')
...
stim_dprimedf %>% group_by(retrieval) %>% summarise_at(vars(fa),list(fa_mean=mean)) %>% 
  ggplot(aes(x=retrieval,y=fa_mean,fill=retrieval)) +geom_bar(stat="identity")+ggtitle('fa retreived vs not retreived stimuli')
...
dprimedf %>% group_by(VVIQ_group) %>% summarise_at(vars(fa),list(fa_mean=mean)) %>% 
  ggplot(aes(x=VVIQ_group,y=fa_mean,fill=VVIQ_group)) + geom_bar(stat='identity') +ggtitle('mean FA rates by VVIQ groups')
print(t.test(dprimedf$fa[dprimedf$VVIQ_group=='high'],dprimedf$fa[dprimedf$VVIQ_group=='mid']))
#this is where I'd put my ttest between high and low subjects... IF I HAD ANY
#dprime
dprimedf %>% group_by(VVIQ_group) %>% summarise_at(vars(dprime),list(dprime_mean=mean)) %>% 
  ggplot(aes(x=VVIQ_group,y=dprime_mean,fill=VVIQ_group)) + geom_bar(stat='identity')+ggtitle('mean dprime by VVIQ groups')
