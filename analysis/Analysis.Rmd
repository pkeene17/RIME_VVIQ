...
stim_dprimedf %>% group_by(retrieval) %>% summarise_at(vars(hr),list(hr_mean=mean)) %>% 
  ggplot(aes(x=retrieval,y=hr_mean,fill=retrieval))+geom_bar(stat="identity")+geom_bar(stat="identity")+
  geom_errorbar(aes(ymin=hr_mean-sd(hr_mean),ymax=hr_mean+sd(hr_mean)),width=.2)+ggtitle('hr retreived vs not retreived stimuli')
print(t.test(stim_dprimedf$hr[stim_dprimedf$retrieval=='not retrieved'],stim_dprimedf$hr[stim_dprimedf$retrieval=='retrieved']))

#plot hr and fa by retrieval status
stim_dprimedf %>% group_by(retrieval) %>% summarise_at(vars(fa),list(fa_mean=mean)) %>% 
  ggplot(aes(x=retrieval,y=fa_mean,fill=retrieval))+
  geom_bar(stat="identity")+geom_errorbar(aes(ymin=fa_mean-sd(fa_mean),ymax=fa_mean+sd(fa_mean)),width=.2)+
...
```{r beh-vividness correlation, echo=FALSE, message=FALSE, warning=FALSE}
...
dprimedf <- mutate(dprimedf,VVIQ_group=unlist(lapply(dprimedf$VVIQ,label_VVIQ)))
dprimedf <- dprimedf[dprimedf$subjID %!in% exclude_subj,]

#plot diff between fa/dprime for high vs low VVIQ subjects
dprimedf %>% group_by(VVIQ_group) %>% summarise_at(vars(fa),list(fa_mean=mean)) %>% 
  ggplot(aes(x=VVIQ_group,y=fa_mean,fill=VVIQ_group)) + geom_bar(stat='identity') +
  geom_errorbar(aes(ymin=fa_mean-sd(fa_mean),ymax=fa_mean+sd(fa_mean)),width=.2)+ggtitle('mean FA rates by VVIQ groups')
print(t.test(dprimedf$fa[dprimedf$VVIQ_group=='high'],dprimedf$fa[dprimedf$VVIQ_group=='mid']))
tmp <- mean(dprimedf$fa[dprimedf$VVIQ_group=='low'])
print(t.test(dprimedf$fa[dprimedf$VVIQ_group=='high'],mu=tmp,alternative='two.sided'))
#print(t.test(dprimedf$fa[dprimedf$VVIQ_group=='mid'],dprimedf$fa[dprimedf$VVIQ_group=='low']))
#dprime
dprimedf %>% group_by(VVIQ_group) %>% summarise_at(vars(dprime),list(dprime_mean=mean)) %>% 
  ggplot(aes(x=VVIQ_group,y=dprime_mean,fill=VVIQ_group)) + geom_bar(stat='identity')+
  geom_errorbar(aes(ymin=dprime_mean-sd(dprime_mean),ymax=dprime_mean+sd(dprime_mean)),width=.2)+ggtitle('mean dprime by VVIQ groups')
print(t.test(dprimedf$dprime[dprimedf$VVIQ_group=='high'],dprimedf$dprime[dprimedf$VVIQ_group=='mid']))
tmp <- mean(dprimedf$dprime[dprimedf$VVIQ_group=='low'])
print(t.test(dprimedf$dprime[dprimedf$VVIQ_group=='high'],mu=tmp,alternative='two.sided'))
```
