function theData = RIME_fMRI_R1(thePath,sNum,S,R1Name,block)    
...
load([thePath '/LIST/retrieve1List.mat']);

theList.Study1      = lists{sNum}.Study1;
theList.Vivid       = lists{sNum}.Vivid;
retrieveLength      = length(retrieve1List{sNum}); %96
...
theData.R1.condition         = retrieve1List{sNum}(1,:);
theData.R1.conID             = retrieve1List{sNum}(2,:);
theData.R1.wordID            = retrieve1List{sNum}(3,:);
theData.R1.imageID           = retrieve1List{sNum}(4,:);
theData.R1.word              = retrieve1List{sNum}(5,:);
theData.R1.image             = retrieve1List{sNum}(6,:);
theData.R1.miniblock         = retrieve1List{sNum}(7,:);
theData.R1.category          = retrieve1List{sNum}(8,:);
theData.R1.image_vers        = retrieve1List{sNum}(9,:); % (_1 or _2 - which ever one is used in this condition in this phase, so I can always read in image_vers and not worry about it later)
theData.R1.image_vers_code 	 = retrieve1List{sNum}(10,:); % code which tells me whether for this subejct the original in study1 had extension _1 or _2
%theData.R1.nonzero           = [];
...
for preall = 1:retrieveLength
    theData.R1.onset(preall) = 0; %<-- this will code the actual measured onset of the trial (here the trial begins with the word), so it should be almost identical to plannedplannedwordonset
    theData.R1.plannedwordonset(preall) = 0; 
    theData.R1.plannedpreoddevenfixonset(preall) = 0; 
    theData.R1.plannedpostoddevenfixonset(preall) = 0; 
end
...
%    Screen('DrawTexture', S.Window, blank); commented out by RS per Hongmi
    ...
    theData.R1.plannedReminderonset(block) = goTime-instrScrTime; 
    ...
    for Trial = 1:48 %changed from retrieveLength RS
            ...
            theData.R1.plannedwordonset(Trial) = goTime - vividStimTime;
            ...
            % Draw word
            word = theData.R1.word{Trial}(3:end);
            ...
            theData.R1.onset(Trial) = realonset; %<---------------------------------------------------- actual trial onset time
...
%                         fclose(fp); sca; return;
            ...
            theData.R1.vividresponse_currblock{blockTrial,1} = vividkey; % FRR replaced repeats with 1 as we do not repeat the rating
            theData.R1.vividRT_currblock{blockTrial,1}       = vividtime;
            theData.R1.vividresponse{Trial,1}    = vividkey;     % this is needed when I am appending the file that saves across blocks
            theData.R1.vividRT{Trial,1}          = vividtime;       
...
%            Present null (fixation)
            ...
            theData.R1.allonsets = [theData.R1.onset; theData.R1.plannedwordonset];%theData.R1.plannedpreoddevenfixonset;theData.R1.plannedpostoddevenfixonset]; %not using odd/even task
            
            eval(['save ' R1Dir '/' R1Name ' theData theList'])
%             eval(['save ' R1Dir '/' R1Name '_study.mat'])
            ...
            block, Trial, theData.R1.plannedwordonset(1,Trial),theData.R1.onset(Trial),....
            cell2mat(theData.R1.vividRT(Trial)), cell2mat(theData.R1.vividresponse(Trial)) );
    ...
    for n = 1:length(theData.R1.vividresponse_currblock);
        if theData.R1.vividresponse_currblock{n} > 0;
            respnum = respnum + 1;
        else
            respnum = respnum;
        end
...
%             eval(['save ' R1Dir '/' R1Name '_study.mat'])
    ...
    respnum = 0;   %initialize in case participant doesn't response at all
    for n = 49:length(theData.R1.vividresponse_currblock);
        if theData.R1.vividresponse_currblock{n} > 0;
            respnum = respnum + 1;
        else
            respnum = respnum;
